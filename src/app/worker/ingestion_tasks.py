...
def _get_pipeline(config, table_name: str):
    ...
    if table_name not in _PIPELINES:
        from app.config.app_config import DEFAULT_EMBEDDING_MODEL
...
async def _chunk_document(doc_id: str) -> Dict[str, Any]:
    ...
    async def work(repo, config, doc):
        from app.config.app_config import DEFAULT_CHUNKING_SIMILARITY
        from app.ingestion.embedding.pipeline import ChunkEmbeddingPipeline
...
@celery_app.task(name="app.worker.ingestion_tasks.parse_document")
def parse_document_task(doc_id: str) -> Dict[str, Any]:
    return _run(_parse_document(doc_id))


@celery_app.task(name="app.worker.ingestion_tasks.chunk_document")
def chunk_document_task(doc_id: str) -> Dict[str, Any]:
    return _run(_chunk_document(doc_id))


@celery_app.task(name="app.worker.ingestion_tasks.embed_document")
