...
from app.config.graph_config import get_graph_config
...
async def get_db_pool() -> asyncpg.Pool:
    ...
    Uses the same ConnectionPoolManager as the rest of the app rather than
    opening its own pool, so graph queries share connections with ingestion
    and retrieval.
    """
    from app.api.dependencies import config
    from app.infra.db import ConnectionPoolManager

    return await ConnectionPoolManager.get_pool(config.connection_string)
